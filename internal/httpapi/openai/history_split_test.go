...
func TestBuildOpenAICurrentInputContextTranscriptUsesNumberedHistorySections(t *testing.T) {
	...
	if !strings.Contains(transcript, "Compiled conversational state and prior function-call outcomes.") {
...
func TestApplyCurrentInputFileUploadsFirstTurnWithNumberedHistoryTranscript(t *testing.T) {
	...
	if !strings.Contains(out.FinalPrompt, "Pick up from the most recent state inside chat_context.txt.") {
...
func TestApplyCurrentInputFilePreservesFullContextPromptForTokenCounting(t *testing.T) {
	...
	if !strings.Contains(out.PromptTokenText, "Pick up from the most recent state inside chat_context.txt.") {
...
func TestApplyCurrentInputFileUploadsToolsContextSeparately(t *testing.T) {
	...
	if strings.Contains(historyText, "The following functions are exposed for this turn") || strings.Contains(historyText, "Synopsis: search docs") {
	...
	if !strings.Contains(out.FinalPrompt, "Pick up from the most recent state inside chat_context.txt.") || !strings.Contains(out.FinalPrompt, "tool_schema.txt") {
	...
	if strings.Contains(out.FinalPrompt, "The following functions are exposed for this turn") || strings.Contains(out.FinalPrompt, "Synopsis: search docs") || strings.Contains(out.FinalPrompt, "Contract:") {
...
func TestChatCompletionsCurrentInputFileUploadsContextAndKeepsNeutralPrompt(t *testing.T) {
	...
	if !strings.Contains(promptText, "Pick up from the most recent state inside chat_context.txt.") {
...
func TestCurrentInputFileWorksAcrossAutoDeleteModes(t *testing.T) {
	for _, mode := range []string{"none", "single", "all"} {
		t.Run(mode, func(t *testing.T) {
			...
			promptText, _ := ds.completionReq["prompt"].(string)
			if !strings.Contains(promptText, "Pick up from the most recent state inside chat_context.txt.") || strings.Contains(promptText, "first user turn") || strings.Contains(promptText, "latest user turn") {
